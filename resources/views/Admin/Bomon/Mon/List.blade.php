  ...
  $(document).ready(function() {
    ...
    $("#xoahet").click(function(){
        ...
        listid=listid.substr(1);   //alert(listid);
          ...
          if (hoi==true) document.location.href = "set_bomon/mon/multi_delete/" + listid;
        });
    });
</script>
    ...
    <div class="row padding20" style="padding-bottom:0;margin-bottom:20px;">
      ...
      {{-- <div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
        ...
        <select id="bomon" class="form-control select2">
          ...
          @foreach ($Bomon as $BM)
          <option value="{{$BM->id}}"
            @if (Session::has('bomon') && Session::get('bomon')==$BM->id)
            selected
          @endif>
          {{$BM->ten_vi}}</option>
          @endforeach
    ...
    <table class="table table-bordered table-hover example2" style="width: 100%">
      <thead>                 
        <tr class="bg-top">
          ...
          <th width="30%" class="text-center">Tên EN</th> 
    ...
    @foreach($Mon as $Mo)                                  
      <tr>
        ...
        <td class="text-center" style="display:inline-flex;"> 
          <a class="btn btn-link btn-warning" href="{{ asset('set_bomon/mon/edit/'.$Mo->id) }}" title="Cập nhật" style="margin-right:10px;">
              <i class="fa fa-edit" style="color:white;"></i>
          </a>
           <a  class="btn btn-link btn-danger" href="{{ asset('set_bomon/mon/one_delete/'.$Mo->id) }}" onClick="if(!confirm('Xác nhận xóa:')) return false;" title="Xóa">
...
<a href="{{ asset('set_bomon/mon/add') }}"><button class=" btn btn-success btn2">Thêm</button></a>
...
$(document).on('change','#bomon',function(){
  ...
  window.location.href ="set_bomon/mon/list/"+bomon+"/"+bac;
});

$(document).on('change','#bacdaotao',function(){
  var bac=$('#bacdaotao').val();
  //alert(bac);
  if(bac==0){alert('Vui lòng chọn bậc đào tạo');return false;}
  // if(bomon==0){alert('Vui lòng chọn bộ môn hợp lệ');return false;}
  window.location.href ="set_bomon/mon/list/"+bac;
});
</script>
@endsection