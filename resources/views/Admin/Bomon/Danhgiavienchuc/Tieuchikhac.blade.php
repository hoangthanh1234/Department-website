...
<form method="post" action="set_bomon/danh-gia-vien-chuc/{{$Phieudanhgia->giangvien->tenkhongdau}}/tieu-chi-khac/3/{{$Phieudanhgia->id}}" enctype="multipart/form-data" >
	...
	<table  class="table table-bordered table-hover" style="width:100%!important">
			<thead>
				<tr class="bg-top" style="width:100%">
					...
					<th width="10%" class="text-center">Góp ý</th>		
					<th width="10%" class="text-center">Phản hồi</th>
			...
			<tbody>				
				...
				@foreach($Tieuchikhac as $Tieuchi)
					<tr>
						...
						<td class="text-center">
							@foreach($Phieudanhgia->phieukhactrong as $pk)
								@if($pk->id_tieuchi == $Tieuchi->id && $pk->gopybomon == '')
									<span class="capnhatgopy" data-id="{{$Tieuchi->id}}" data-ten="{{$Tieuchi->ten}}" data-loai="{{$Tieuchi->loaiminhchung}}" data-gopy="{{$pk->gopybomon}}">Thêm</span>
								@endif

								@if($pk->id_tieuchi == $Tieuchi->id && $pk->gopybomon != '')
									<span class="capnhatgopy capnhatgopy{{$Tieuchi->id}}" data-gopy="{{$pk->gopybomon}}" data-id="{{$Tieuchi->id}}" data-ten="{{$Tieuchi->ten}}" data-loai="{{$Tieuchi->loaiminhchung}}" data-gopy="{{$pk->gopybomon}}">Cập nhật</span>
								@endif
							@endforeach
						</td>
						...
						<td class="text-center">
							@foreach($Phieudanhgia->phieukhactrong as $pk)
								@if($pk->id_tieuchi == $Tieuchi->id && $pk->phanhoibomon != '')
									<span class="phanhoibomon" data-id="{{$Tieuchi->id}}" data-ten="{{$Tieuchi->ten}}" data-loai="{{$Tieuchi->loaiminhchung}}" data-phanhoi="{{$pk->phanhoibomon}}">Có</span>
								@endif
							@endforeach
						</td>	
...
<script type="text/javascript">
	...
	$(document).on('click','.btn-capnhatminhchungmoi',function(){
		...
		if(type==1){
			 $.ajax({
				    ...
				    method:'POST',
				    url:'set_bomon/ajax/uploadminhchungkhaclink',
				    data:{
		...
		if(type==2){
            ...
            form_data.append('_token', token);
			 ...
			 $.ajax({
				    url: 'set_bomon/ajax/uploadminhchungkhacfile', // point to server-side PHP script
				    data:form_data,
				    type: 'POST',
	...
	$(document).on('change','.mycheck',function(){
		...
		if($(this).is(':checked')){
			...
			$.ajax({
				method:'POST',
				url:'set_bomon/ajax/huy-duyet/khac',
				data:{
				    id_tieuchi:id,
				    id_phieu:id_phieu,				   
				    _token:token
	...
	$(document).on('click','#btn-luu-gopy',function(){
		...
		var gopy=$('#gopy').val();
		 ...
		 $.ajax({
				...
				method:'POST',
				url:'set_bomon/ajax/cap-nhat-gop-y/khac',
				data:{
				    id_tieuchi:id_tieuchi,
				    id_phieu:id_phieu,
				    gopy:gopy,	
...
$(document).ready(function(){
	...
	@foreach($Phieudanhgia->phieukhacbomonduyet as $pk)			
		$('#diemdat<?=$pk->id_tieuchi?>').val('<?=$pk->diemdatbomon?>');		
	@endforeach
	$('#tongdiem').val(<?=$Phieudanhgia->phieukhacbomonduyet->sum('diemdatbomon')?>);
});
