...
<form method="post" action="set_giangvien/danh-gia-vien-chuc/giang-day/1" enctype="multipart/form-data" >
			...
			<tbody>				
				...
				@foreach($Tieuchigiangday as $Tieuchi)
					<tr>
						...
						<td>
							<input type="text" class="form-control" name="ghichu{{$Tieuchi->id}}" 
				...
				@if($bmgy->phanhoibomon == '')
					<div class="row" style="border-bottom:1px dashed #777;padding-top:10px">
						...
						<div class="col-lg-5 col-md-6 col-xs-12" style="padding-right:0;">						
							<input type="text" class="form-control" id="phanhoibomon{{$bmgy->id}}" value="{{$bmgy->phanhoibomon}}"  placeholder="Nhập phản hồi tại đây">	
						</div>
						<div class="col-lg-2"><button type="button" class="btn btn-success btn-phanhoibomon" data-id="{{$bmgy->id}}">Phản hồi</button></div>
					</div>
				@endif
				@endforeach	        	
	        	...
	        	@if($kgy->phanhoikhoa== '')
					<div class="row" style="border-bottom:1px dashed #777;padding-top:10px">
						...
						<div class="col-lg-5 col-md-5 col-xs-10" style="padding-right:0;">						
							<input type="text" class="form-control" id="phanhoikhoa{{$kgy->id}}" value="{{$kgy->phanhoikhoa}}"  placeholder="Nhập phản hồi tại đây">	
						</div>					
						<div class="col-lg-2 col-md-2 col-xs-2"><button type="button" class="btn btn-success btn-phanhoikhoa" data-id="{{$kgy->id}}">Phản hồi</button></div>
					</div>
				@endif
				@endforeach	        		
			</div>
...
<script type="text/javascript">
	...
	$(document).on('change','.soluong',function(){
		...
		//value2 = value.substring(1,value.length-1);
	...
	$(document).ready(function(){
		...
		@endforeach
		$('#tongdiem').val(<?=$Phieudanhgia->phieugiangday->sum('diemdat');?>);
	});
	...
	$(document).on('click','.btn-capnhatminhchungmoi',function(){
		...
		if(type==1){
			 $.ajax({
				    ...
				    method:'POST',
				    url:'set_giangvien/ajax/uploadminhchunggiangdaylink',
				    data:{
		...
		if(type==2){
            ...
            form_data.append('_token', token);
			 ...
			 $.ajax({
				    url: 'set_giangvien/ajax/uploadminhchunggiangdayfile', // point to server-side PHP script
				    data:form_data,
				    type: 'POST',
	...
	$(document).on('click','.btn-phanhoibomon',function(){
		var id =$(this).data("id");
		var value=$('#phanhoibomon'+id).val();
        ...
        $.ajax({
			url: 'set_giangvien/ajax/phanhoibomon', // point to server-side PHP script
			data:form_data,
			type: 'POST',
			contentType: false, // The content type used when sending data to the server.
	...
	$(document).on('click','.btn-phanhoikhoa',function(){
		var id =$(this).data("id");
		var value=$('#phanhoikhoa'+id).val();
        ...
        $.ajax({
			url: 'set_giangvien/ajax/phanhoikhoa', // point to server-side PHP script
			data:form_data,
			type: 'POST',
			contentType: false, // The content type used when sending data to the server.
			cache: false, // To unable request pages to be cached
