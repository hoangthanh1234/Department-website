...
<form method="post" action="set_admin/danh-gia-vien-chuc/{{$Phieudanhgia->giangvien->tenkhongdau}}/giang-day/1/{{$Phieudanhgia->id}}" enctype="multipart/form-data" >
	...
	<table  class="table table-bordered table-hover" style="width:100%!important">
			<thead>
				<tr class="bg-top" style="width:100%">
					...
					<th width="35%">Tên tiêu chí</th>				
					<th width="5%" class="text-center">ĐVT</th>
					<th width="3%" class="text-center">Điểm</th>						
					<th width="8%" class="text-center">Số lượng</th>	
					<th width="3%" class="text-center">Điểm</th>
					<th width="8%" class="text-center">Ghi chú</th>
					<th width="7%" class="text-center">Góp ý</th>
					<th width="7%" class="text-center" title="Bộ môn góp Ý">BMGY</th>		
					<th width="5%" class="text-center" title="Phản hồi BM">PH</th>
					<th width="5%" class="text-center">Chọn</th>
					<th width="8%" class="text-center" title="Bộ môn duyệt">BMD</th>
					<th width="5%" class="text-center">Duyệt</th>
					<th width="10%" class="text-center" title="Minh chứng">MC</th>											
					<th width="8%" class="text-center" title="Điểm đạt">ĐĐ</th>
				</tr>
			</thead>
			...
			<tbody>				
					...
					<tr>
						...
						<td class="text-center">							
							...
							@foreach($Phieudanhgia->phieugiangdaytrong as $pgd)
								@if($pgd->id_tieuchi==$Tieuchi->id && $pgd->soluongkhoa == 0) 
									@if($pgd->id_tieuchi==$Tieuchi->id && $pgd->soluong != 0)
										value="{{$pgd->soluong}}"
									@endif
						...
						<td class="text-center">
							@foreach($Phieudanhgia->phieugiangdaytrong as $pgd)
								@if($pgd->id_tieuchi == $Tieuchi->id && $pgd->gopykhoa == '')
									<span class="capnhatgopy" data-id="{{$Tieuchi->id}}" data-ten="{{$Tieuchi->ten}}" data-loai="{{$Tieuchi->loaiminhchung}}" data-gopy="{{$pgd->gopykhoa}}" id="capnhatgopy{{$Tieuchi->id}}">Thêm</span>
								@endif

								@if($pgd->id_tieuchi == $Tieuchi->id && $pgd->gopykhoa != '')
									<span class="capnhatgopy capnhatgopy{{$Tieuchi->id}}" data-gopy="{{$pgd->gopykhoa}}" data-id="{{$Tieuchi->id}}" data-ten="{{$Tieuchi->ten}}" data-loai="{{$Tieuchi->loaiminhchung}}" data-gopy="{{$pgd->gopykhoa}}" id="capnhatgopy{{$Tieuchi->id}}">Cập nhật</span>
								@endif
							@endforeach
						</td>
						...
						<td class="text-center">
							@foreach($Phieudanhgia->phieugiangdaytrong as $pgd)
								@if($pgd->id_tieuchi == $Tieuchi->id && $pgd->gopybomon != '')
									<span class="gopybomon gopybomon{{$Tieuchi->id}}" data-gopy="{{$pgd->gopybomon}}" data-id="{{$Tieuchi->id}}" data-ten="{{$Tieuchi->ten}}" data-loai="{{$Tieuchi->loaiminhchung}}" data-gopy="{{$pgd->gopybomon}}">Xem</span>
								@endif
							@endforeach
						</td>
						...
						<td class="text-center">
							@foreach($Phieudanhgia->phieugiangdaytrong as $pgd)
								@if($pgd->id_tieuchi == $Tieuchi->id && $pgd->phanhoikhoa != '')
									<span class="phanhoikhoa" data-id="{{$Tieuchi->id}}" data-ten="{{$Tieuchi->ten}}" data-loai="{{$Tieuchi->loaiminhchung}}" data-phanhoi="{{$pgd->phanhoikhoa}}">Có</span>
								@endif
							@endforeach
						</td>		
						...
						<td class="text-center">
							...
							@foreach($Phieudanhgia->phieugiangdaykhoaduyet as $pgd)
								@if($pgd->id_tieuchi == $Tieuchi->id)
									checked
								@endif
							@endforeach
							...
							@foreach($Phieudanhgia->phieugiangdaytrong as $pgd )
								@if($pgd->id_tieuchi==$Tieuchi->id)
									@if($pgd->minhchung!='')
										...
										@foreach($soco as $sc)
										<a href="@if(substr($sc,0,5)=='drive') set_admin/ajax/get/{{$sc}}@else {{$sc}} @endif" target="_blank">Có_{{$dem++}}</a>										
										@endforeach
										<br>		
				...
				@endforeach
				<tr>
					<td colspan="12" class="text-center"><b class="ten2x">Tổng điểm:</b></td>
...
<div class="modal fade" id="gopybomonmodal" role="dialog">
	<div class="modal-dialog modal-md"> 
	    <div class="modal-content">
	        <div class="modal-header">
	          <button type="button" class="close" data-dismiss="modal">&times;</button>
	          <h4 class="modal-title ten2x" style="font-size:20px;">XEM GÓP Ý TỪ BỘ MÔN</h4>	        
	        </div>
	        <div class="modal-body">
	        	<p class="boldphieu1" style="font-weight:normal;font-size:16px;">Tiêu chí:&nbsp; &nbsp;<span id="tentieuchibomon" ></span></p>
	        	<p class="boldphieu1" style="font-weight:normal;font-size:16px;">Loại minh chứng:&nbsp; &nbsp;<span id="loaiminhchungbomon" ></span></p>
	        	<div class="row">
	        		 <div class="form-group">
		        		<div class="col-lg-12 col-md-6 col-sm-6 col-xs-12">
		        			<textarea class="form-control" id="gopybomon" disabled></textarea>
		        		</div>		        		
	        		</div>	        		
	        	</div>	        		        		
			</div>
	        ...
	        <div class="modal-body">
	        	...
	        	<div class="row">
	        		 <div class="form-group" style="margin-bottom:25px;">
	        		 	<div class="col-lg-12">
	        		 		<input type="radio" name="r3" checked class="flat-red" value="1">&nbsp;&nbsp;Link liên kết đến minh chứng
	        		 	</div>
	        		</div>
	        		 <div class="form-group">
...
<script type="text/javascript">
	...
	$(document).on('click','.btn-capnhatminhchungmoi',function(){
		...
		if(type==1){
			 ...
			 $.ajax({
				    ...
				    method:'POST',
				    url:'set_admin/ajax/uploadminhchunggiangdaylink',
				    data:{
		...
		if(type==2){
            ...
            form_data.append('_token', token);
			 ...
			 $.ajax({
				    url: 'set_admin/ajax/uploadminhchunggiangdayfile', // point to server-side PHP script
				    data:form_data,
				    type: 'POST',
	...
	$(document).on('change','.mycheck',function(){
		...
		if($(this).is(':checked')){
			...
			$.ajax({
				method:'POST',
				url:'set_admin/ajax/huy-duyet/giang-day',
				data:{
				    id_tieuchi:id,
				    id_phieu:id_phieu,				   
				    _token:token
	...
	$(document).on('click','.gopybomon',function(){
		var id_tieuchi=$(this).data('id');
		$('#tentieuchibomon').html($(this).data('ten'));
		$('#loaiminhchungbomon').html($(this).data('loai'));			
		$('#gopybomon').val($(this).data('gopy'));	
		$('#gopybomonmodal').modal();
	});
	...
	$(document).on('click','#btn-luu-gopy',function(){
		...
		var gopy=$('#gopy').val();
		 ...
		 $.ajax({
				...
				method:'POST',
				url:'set_admin/ajax/cap-nhat-gop-y/giang-day',
				data:{
				    id_tieuchi:id_tieuchi,
				    id_phieu:id_phieu,
				    gopy:gopy,	
				...
				success: function(data){  
					...
					if(data == ''){
						$('#capnhatgopy'+id_tieuchi).html('Thêm');
						$('#capnhatgopy'+id_tieuchi).data('gopy','');
					}
					else{
						$('#capnhatgopy'+id_tieuchi).html('Cập nhật');
						$('#capnhatgopy'+id_tieuchi).data('gopy',data);
					}
				    //window.location.reload();   
				},
				error: function(XMLHttpRequest, textStatus, errorThrown){                     
	...
	$(document).on('click','.phanhoikhoa',function(){
		var id_tieuchi=$(this).data('id');
...
$(document).ready(function(){
	var tong=0;
	@foreach($Phieudanhgia->phieugiangdaykhoaduyet as $pgd)				
		$('#diemdat<?=$pgd->id_tieuchi?>').val('<?=$pgd->diemdatkhoa?>');					
	@endforeach
	$('#tongdiem').val(<?=$Phieudanhgia->phieugiangdaykhoaduyet->sum('diemdatkhoa')?>);
});
