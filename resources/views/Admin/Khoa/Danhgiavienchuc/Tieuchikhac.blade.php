...
<form method="post" action="set_admin/danh-gia-vien-chuc/{{$Phieudanhgia->giangvien->tenkhongdau}}/tieu-chi-khac/3/{{$Phieudanhgia->id}}" enctype="multipart/form-data" >
	...
	<table  class="table table-bordered table-hover" style="width:100%!important">
			<thead>
				<tr class="bg-top" style="width:100%">
					...
					<th width="7%" class="text-center">BM Góp ý</th>		
					<th width="7%" class="text-center">Phản hồi</th>
				...
				@foreach($Tieuchikhac as $Tieuchi)
					<tr>
						...
						<td class="text-center">							
							...
							@foreach($Phieudanhgia->phieukhactrong as $pk)
								@if($pk->id_tieuchi==$Tieuchi->id && $pk->soluongkhoa == 0) 
									@if($pk->id_tieuchi==$Tieuchi->id && $pk->soluong != 0)
										value="{{$pk->soluong}}"
									@endif
									@if($pk->id_tieuchi==$Tieuchi->id && $pk->soluongbomon != 0)
										value="{{$pk->soluongbomon}}"
									@endif
								@endif

								@if($pk->id_tieuchi==$Tieuchi->id && $pk->soluongkhoa != 0) 
									value="{{$pk->soluongkhoa}}"
								@endif
							@endforeach>
						</td>
						...
						<td class="text-center">
							@foreach($Phieudanhgia->phieukhactrong as $pk)
								@if($pk->id_tieuchi == $Tieuchi->id && $pk->gopykhoa == '')
									<span class="capnhatgopy" data-id="{{$Tieuchi->id}}" data-ten="{{$Tieuchi->ten}}" data-loai="{{$Tieuchi->loaiminhchung}}" data-gopy="{{$pk->gopykhoa}}" id="capnhatgopy{{$Tieuchi->id}}">Thêm</span>
								@endif

								@if($pk->id_tieuchi == $Tieuchi->id && $pk->gopykhoa != '')
									<span class="capnhatgopy capnhatgopy{{$Tieuchi->id}}" data-gopy="{{$pk->gopykhoa}}" data-id="{{$Tieuchi->id}}" data-ten="{{$Tieuchi->ten}}" data-loai="{{$Tieuchi->loaiminhchung}}" data-gopy="{{$pk->gopykhoa}}" id="capnhatgopy{{$Tieuchi->id}}">Cập nhật</span>
								@endif
							@endforeach
						</td>
						...
						<td class="text-center">
							@foreach($Phieudanhgia->phieukhactrong as $pk)
								@if($pk->id_tieuchi == $Tieuchi->id && $pk->gopybomon != '')
									<span class="gopybomon gopybomon{{$Tieuchi->id}}" data-gopy="{{$pk->gopybomon}}" data-id="{{$Tieuchi->id}}" data-ten="{{$Tieuchi->ten}}" data-loai="{{$Tieuchi->loaiminhchung}}" data-gopy="{{$pk->gopybomon}}">Xem</span>
								@endif
							@endforeach
						</td>
						...
						<td class="text-center">
							@foreach($Phieudanhgia->phieukhactrong as $pk)
								@if($pk->id_tieuchi == $Tieuchi->id && $pk->phanhoikhoa != '')
									<span class="phanhoikhoa" data-id="{{$Tieuchi->id}}" data-ten="{{$Tieuchi->ten}}" data-loai="{{$Tieuchi->loaiminhchung}}" data-phanhoi="{{$pk->phanhoikhoa}}">Có</span>
								@endif
							@endforeach
						</td>	
						...
						<td class="text-center">
							...
							@foreach($Phieudanhgia->phieukhac as $pk)
								@if($pk->id_tieuchi == $Tieuchi->id)
									checked
								@endif
							@endforeach>
						</td>
						...
						<td class="text-center">
							<input type="checkbox" disabled="disabled" 
							@foreach($Phieudanhgia->phieukhacbomonduyet as $pk)
								@if($pk->id_tieuchi == $Tieuchi->id)
									checked
								@endif
							@endforeach>
						...
						<td class="text-center">
							...
							@foreach($Phieudanhgia->phieukhackhoaduyet as $pk)
								@if($pk->id_tieuchi == $Tieuchi->id)
									checked
								@endif
							@endforeach
						...
						<td class="text-center">
							@foreach($Phieudanhgia->phieukhactrong as $pk )
								@if($pk->id_tieuchi==$Tieuchi->id)
									@if($pk->minhchung!='')
										<?php $soco= explode(",", $pk->minhchung);$dem=1;?>
										@foreach($soco as $sc)
...
<script type="text/javascript">
	...
	$(document).on('click','.btn-capnhatminhchungmoi',function(){
		...
		if(type==1){
			 $.ajax({
				    ...
				    method:'POST',
				    url:'set_admin/ajax/uploadminhchungkhaclink',
				    data:{
		...
		if(type==2){
            ...
            form_data.append('_token', token);
			 ...
			 $.ajax({
				    url: 'set_admin/ajax/uploadminhchungkhacfile', // point to server-side PHP script
				    data:form_data,
				    type: 'POST',
	...
	$(document).on('change','.mycheck',function(){
		...
		if($(this).is(':checked')){
			...
			$.ajax({
				method:'POST',
				url:'set_admin/ajax/huy-duyet/khac',
				data:{
				    id_tieuchi:id,
				    id_phieu:id_phieu,				   
				    _token:token
	...
	$(document).on('click','#btn-luu-gopy',function(){
		...
		var gopy=$('#gopy').val();
		 ...
		 $.ajax({
				...
				method:'POST',
				url:'set_admin/ajax/cap-nhat-gop-y/khac',
				data:{
				    id_tieuchi:id_tieuchi,
				    id_phieu:id_phieu,
				    gopy:gopy,	
	...
	$(document).ready(function(){		
		@foreach($Phieudanhgia->phieukhackhoaduyet as $pk)			
			$('#diemdat<?=$pk->id_tieuchi?>').val('<?=$pk->diemdatkhoa?>');			
		@endforeach
		$('#tongdiem').val(<?=$Phieudanhgia->phieukhackhoaduyet->sum('diemdatkhoa')?>);
	});
