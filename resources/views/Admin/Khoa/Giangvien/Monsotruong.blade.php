...
<div class="h3 padding20 text-center">Danh sách môn sở trường của giảng viên giảng viên</div>
    <div class="box">   
                            ...
                            <tbody>   
                                ...
                                @foreach($Monsotruong as $mst)                                  
                                    <tr> 
                                        ...
                                        <td class="text-center">{{$mst->mon->bacdaotao->ten_vi}}</td>
                                        <td class="text-center">{{$mst->mon->chuyennganh->ten_vi}}</td>
                                        <td class="text-center">{{$mst->mon->bomon->ten_vi}}</td>
                                        </tr>  
                                  @endforeach                          
                                  </tbody> 
                            </table> 
    ...
    <a href="set_admin/giangvien/list/0"><button class=" btn btn-danger  btn2">Thoát</button></a>
...
@section('script')
	<script type="text/javascript">
		$(document).on('click','#luumonthem',function(){
			...
			var id_giangvien = <?=$id_giangvien?>;
			$.ajax({
	            url: "set_admin/ajax/luumonsotruong/"+id_mon+"/"+id_giangvien,
	            type: 'GET',
	            dataType: 'html',  
		...
		$(document).on('click','.xoamonsotruong',function(){
			...
			$.ajax({
	            url: "set_admin/ajax/xoamonsotruong/"+id_mon+"/"+id_giangvien,
	            type: 'GET',
	            dataType: 'html',  
	            success: function(data){                      
	               alert(data); window.location.reload();                
