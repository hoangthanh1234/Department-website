                        ...
                        <tbody>   
                          @foreach($Giangvien->quatrinhcongtac as $congtac)                                  
                            <tr>
						        ...
						        <td class="text-center">                                                   
                                    <i class="fa fa-2x fa-edit blue" data-toggle="modal" data-target="#capnhatcongtac{{$congtac->id}}"></i>
                                </td> 

                                <td class="text-center" title="xóa bài viết"> 
                                 	<i class="fa fa-times text-dange fa-2x xoacongtac red" data-id="{{$congtac->id}}"></i>
                                </td>
									...
									<div class="row">
										<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
											<div class="form-group">
												...
												<input type="text" id="sodienthoai{{$congtac->id}}" value="{{$congtac->sodienthoai}}"  class="form-control" placeholder="nhập số điện thoại cơ quan">
											</div>
										</div>
...
<script type="text/javascript">
		...
		$('#btn-luu').on('click',function(){
                ...
                $.ajax({
				        method:'POST',
				        url:'set_admin/ajax/themcongtac',
				        data:{
...
$('.btn-luu').on('click',function(){
    ...
    var ghichu=$('#ghichu'+id).val(); 
     ...
     $.ajax({
				...
				method:'POST',
				url:'set_admin/ajax/capnhatcongtac',
				data:{
				        tencoso_vi:tencoso_vi,
				        tencoso_en:tencoso_en,
				        diachicoso_vi:diachicoso_vi,
...
$('.xoacongtac').on('click',function(){
        ...
        var id=$(this).data('id');
        $.get("set_admin/ajax/xoacongtac/"+id,function(data){alert(data);window.location.reload();});
});
...
$('#xoahet').on('click',function(){
    ...
    if (hoi==true){$.get("set_admin/ajax/xoacongtachet/"+listid,function(data){alert(data);window.location.reload();});} 
});
