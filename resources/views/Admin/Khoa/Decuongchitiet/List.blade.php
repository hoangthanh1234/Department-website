...
@section('title','danh sách đề cương chi tiết')
@section('content')  
...
$("#xoahet").click(function(){
  ...
  if (listid=="") { alert("Bạn chưa chọn mục nào"); return false;}
    ...
    if (hoi==true) document.location.href = "set_admin/decuongchitiet/multi_delete/" + listid;
  });
});
</script>
<div class="h3 padding20 text-center">Danh sách Đề cương chi tiết</div>
  <div class="box">       
...
<div class="box-body">              
  ...
  <div class="table-responsive">                          
    <table id="example2" class="table table-bordered table-hover" style="min-width:1200px;width: 100%">
      ...
      <tbody>   
      @foreach($Decuongchitiet as $kqt)                                  
        <tr>
          <td class="text-center">
            <input type="checkbox" name="chon" id="chon" value="{{$kqt->id}}" class="chon" />
          </td>  
          <td>{{$kqt->ten}}</td> 
          <td class="text-center"><a href="ht96_decuongchitiet/{{$kqt->file}}" target="_blank">OK</a></td> 
          <td class="text-center" title="xóa bài viết">
            <a href="set_admin/decuongchitiet/one_delete/{{$kqt->id}}" onClick="if(!confirm('Xác nhận xóa:')) return false;"><i class="fa fa-times text-dange fa-2x"></i></a>
          </td> 
        </tr> 
      @endforeach                          
      </tbody> 
...
<a href="set_admin/decuongchitiet/add"><button class=" btn btn-success btn2">Thêm</button></a>
...
$(document).on('change', '#lop', function(){
  var id=$(this).val();             
   window.location.href ='set_admin/decuongchitiet/list/'+id;             
});
</script>
@endsection
