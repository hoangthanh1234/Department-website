...
<div class="bomon-cuusinhvien">
        ...
        <div class="container-fluid">          
            <div class="row dong1">
                ...
                <div class="col-lg-4 col-md-12 col-sm-12 col-xs-12" style="margin:5px;">
                 ...
                 <div class="col-lg-4 col-md-12 col-sm-12 col-xs-12  dong text-left">
                ...
                <div class="col-lg-2 col-md-2 col-sm-6 col-xs-12  bt-timkiem2" style="display:none;@if(Session::has('Csv')) display:block; @endif">                  
                         <div class="dropdown text-center">
                                ...
                                <ul class="dropdown-menu" style="top:40px;left:20px;">
                                      ...
                                      <li><a href="google/logout" style="font-weight:bold;">Đăng xuất</a></li>
    ...
    <div class="container">
      ...
      <h2 class="bold" style="font-size:25px;">{{trans('Menu.cuusinhvien')}}</h2>
                  ...
                  <table class="table table-striped table-bordered" style="width:100%;min-width:1000px;">
                    <thead>
                      <tr class="bg-top-table">
                        <th class="text-center bold" width="10%">MSSV</th>
                         <th class="text-center bold" width="20%">{{trans('Nhansu.hovaten')}}</th>
                        <th class="text-center bold">{{trans('Cuusinhvien.lop')}}</th>                       
                        <th class="text-center bold" width="15%">{{trans('Cuusinhvien.khoahoc')}}</th>
                        <th class="text-center bold" width="15%">{{trans('Cuusinhvien.namtonghiep')}}</th>
                        <th class="text-center bold" width="10%">{{trans('Public.xemthem')}}</th>
                      </tr>
                    </thead>
                    <tfoot>
                      <th></th>
                                                      ...
                                                      <div class="modal-body">                                                        
                                                       ...
                                                       <div class="row">
                                                          <div class="col-lg-5 col-md-6 col-sm-6 col-xs-12">
                                                               <p class="bold"><label class="ten2x">Họ tà tên:&nbsp;</label>{{$Csv->tensinhvien}}</p> 
                                                              <p class="bold"> <label class="ten2x">Ngày Sinh:&nbsp;</label>{{date('d-m-Y', strtotime($Csv->ngaysinh))}}</p>
                                                              <p class="bold"><label class="ten2x">Địa chỉ:&nbsp;</label>{{$Csv->diachi}}</p>
                                                              <p class="bold"><label class="ten2x">MSSV:&nbsp;</label>{{$Csv->masinhvien}}</p>
                                                              <p class="bold"><label class="ten2x">Bộ môn:&nbsp;</label>{{$Csv->tenbm}}</p>
                                                              <p class="bold"><label class="ten2x">Lớp:&nbsp;</label>{{$Csv->tenlop}}</p>
                                                              
                                                          </div>

                                                          <div class="col-lg-7 col-md-6 col-sm-6 col-xs-12">
                                                             <p class="bold"><label class="ten2x">E-mail:&nbsp;</label>{{$Csv->email}}</p>
                                                             <p class="bold"><label class="ten2x">Điện thoại:&nbsp;</label>{{$Csv->dienthoai}}</p>
                                                             <p class="bold"><label class="ten2x">Nơi Công tác:&nbsp;</label>{{$Csv->noicongtac}}</p>
                                                              <p class="bold"><label class="ten2x">Chức vụ:&nbsp;</label>{{$Csv->chucvu}}</p>
                                                              <p class="bold"><label class="ten2x">Sô cơ quan:&nbsp;</label>{{$Csv->socoquan}}</p>
                                                              <p class="bold"><label class="ten2x">Địa chỉ cơ quan:&nbsp;</label>{{$Csv->diachicoquan}}</p>
                                                          </div>
                                                        </div>
...
<script type="text/javascript">
  $(document).on('click','#btn',function(){
    ...
    if(value==""){alert("Vui lòng nhập giá trị");return false;}
      if(selValue=='lop'){
       window.location="{{trans('Link.cuusinhvien')}}/{{trans('Link.timkiem')}}/{{trans('Link.lop')}}="+value+".html";
      }

      if(selValue=='ten'){
        window.location="{{trans('Link.cuusinhvien')}}/{{trans('Link.timkiem')}}/{{trans('Link.ten')}}="+value+".html";
      }


      if(selValue=='bomon'){
        window.location="{{trans('Link.cuusinhvien')}}/{{trans('Link.timkiem')}}/{{trans('Link.bomon')}}="+value+".html";
      }
  });
  ...
  $(document).on('click','#btn-luu',function(){
      ...
      $.ajax({
                ...
                success: function(data){                      
                                 alert(data);window.location="google/logout";
                 },
